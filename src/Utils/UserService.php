...
class UserService
{
    ...
    public function __construct(Container $container,EntityManagerInterface $entityManager,SerializerInterface $serializer,ValidatorInterface $validator)
    {
            ...
            $errors = $this->validator->validate($user);
             if (count($errors) > 0) {
                foreach($errors as $error){
                    $obj = new \stdClass;
                    $obj->message = $error->getMessage()."(".$error->GetPropertyPath().")";
                    $errorsMessage[] = $obj;
                }
    ...
    {   
        ...
        if (!$user) {
            $message = ["message"=> $translator->trans('api.userService.id_unexist').$id];
            return new Response(
                json_encode($message),
                Response::HTTP_NOT_FOUND,
                ['Content-type' => 'application/json']
    ...
    public function deleteUserById(array $params) : Response
    {
        ...
        $user = $repository->find($id);
            if (!$user) {
                $message = ["message" => $translator->trans('api.userService.unexisting_user').$id];
                return new Response(
                json_encode($message),
                Response::HTTP_NOT_FOUND,
                ['Content-type' => 'application/json']
    ...
    public function changeUserById(array $params) : Response
    {
        ...
        {
            $message = ["message" => $translator->trans('api.userService.empty_fields').implode(",",$parameters)];
            return new Response(
                json_encode($message),
                Response::HTTP_NOT_FOUND,
                ['Content-type' => 'application/json']
